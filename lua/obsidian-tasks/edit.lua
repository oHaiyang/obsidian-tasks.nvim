...
local function ensure_completion_options(buf)
	local current = vim.api.nvim_get_option_value("completeopt", { buf = buf })
	local seen = {}
	local values = {}
	for value in (current or ""):gmatch("[^,]+") do
		if value ~= "" and not seen[value] then
			table.insert(values, value)
			seen[value] = true
		end
	end
	for _, value in ipairs({ "menuone", "noinsert", "noselect" }) do
...
function M.complete(findstart, base)
	...
	return M.suggest_field(ctx.field, base, M.form_state[vim.api.nvim_get_current_buf()] or {})
end
...
local function open_form(fields, state)
	...
	vim.api.nvim_set_option_value("completefunc", "v:lua.require'obsidian-tasks.edit'.complete", { buf = buf })
	ensure_completion_options(buf)
	...
	vim.api.nvim_create_autocmd("TextChangedI", {
		...
		callback = function()
			...
			if form_state.last_completion_line_length and ctx.line_length < form_state.last_completion_line_length then
			...
			vim.schedule(function()
				if vim.api.nvim_get_current_buf() == buf and vim.api.nvim_get_mode().mode:match("^i") then
