...
local function remove_task_id(body)
	return trim((body or ""):gsub("%s*🆔%s*[%w_%-]+", ""))
end

local function remove_depends_on(body)
...
function M.next_task(task, opts)
	...
	body = remove_task_id(body)
	body = remove_depends_on(body)
	...
	for _, field in ipairs(DATE_FIELDS) do
		...
		if next_value then
			body = replace_date(body, field, next_value)
			advanced_any = true
	...
	if opts.remove_scheduled_date_on_recurrence or opts.removeScheduledDateOnRecurrence then
		if has_start_or_due then
			body = remove_date(body, "scheduled")
		end
	end
	...
	if opts.set_created_date or opts.setCreatedDate then
		...
		local replaced
		body, replaced = replace_date(body, "created", today)
